...
import { ChatArchiveEvent, GetChatsQuery } from '../../../structures/chats.dto';
import {
  ChatRequest,
...
import { Label, LabelID } from '../../../structures/labels.dto';
...
export class WhatsappSessionWebJSCore extends WhatsappSession {
  ...
  async getScreenshot(): Promise<Buffer> {
    if (this.status === WAHASessionStatus.FAILED) {
        throw new Error(`The session under FAILED status. Please try to restart it.`);
    }
